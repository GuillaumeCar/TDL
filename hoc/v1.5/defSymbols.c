...
void installDefaultSymbols(void) {
    ...
    for (i = 0; _consts[i].cName != NULL; i++) {
        if (_consts[i].cType == ENTIER) {
            ...
            *pInt = (int)_consts[i].cValue;
            installSymbol(_consts[i].cName, CST, _consts[i].cType, _consts[i].cDesc, sizeof(int), pInt);
        } else if (_consts[i].cType == REEL) {
            pFlo = (double *)malloc(sizeof(double));
            *pFlo = _consts[i].cValue;
            installSymbol(_consts[i].cName, CST, _consts[i].cType, _consts[i].cDesc, sizeof(double), pFlo);
        }
    }
    ...
    for (i = 0; _preDefs[i].pdName != NULL; i++) {
        installSymbol(_preDefs[i].pdName, PRG, PREDEF, _preDefs[i].pdDesc, sizeof(double), (generic)_preDefs[i].pdPtrFunc);
    }

    printMessage(5);
    for (i = 0; _opExpr[i].opName != NULL; i++) {
        installSymbol(_opExpr[i].opName, PRG, _opExpr[i].opType, _opExpr[i].opDesc, sizeof(double), (generic)_opExpr[i].opPtrFunc);
    }
    printMessage(9);
}
